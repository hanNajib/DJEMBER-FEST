...
export default function ArticleFormData({img, judul, deskripsi, IsOpen, setIsOpen}) {
    return(
        <div className={`relative w-screen ${IsOpen ? '' : 'hidden'}`}>
            <div className="overlay w-screen h-screen bg-black opacity-50 fixed z-40 top-0 overflow-x-hidden" onClick={() => setIsOpen(false)}></div>
            <div className="w-screen h-[95vh] scale-[.82] mt-8 fixed bg-slate-200 top-0 z-50 flex flex-col p-3 shadow-2xl rounded-md">
                ...
                <div className="">
                <img src={img} alt={judul} className="w-full h-[30%] md:h-[50%] lg:h-[35%] object-cover z-20 rounded-md"/>
